		...
		<div class="container-fluid">
			...
			<div class="row justify-content-center">
					<div class="col-sm-12 col-md-10">
							...
							<form class="form-horizontal" method="POST" action="{{ route('unit_usaha.update', ['id' => $row->id]) }}" enctype="multipart/form-data">
								@csrf
								@method("PUT")
								<div class="panel panel-default">
										<div class="panel-heading text-muted">Unit Usaha <span class="label label-info m-l-5">Edit</span></div>
                                                        ...
                                                        <div class="col-sm-12 col-md-10">
                                                            <select name="id_jenis" class="form-control">
                                                                @foreach($jenis_unit_usaha as $e)
                                                                    <option value="{{ $e->id }}"
                                                                        @if($row->id_jenis == $e->id) selected @endif
                                                                        >{{ $e->nama }}</option>
                                                                @endforeach
                                                            </select>
